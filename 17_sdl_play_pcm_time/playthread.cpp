...
#define WAVFILENAME "/Users/cloud/Documents/iOS/音视频/TestMusic/out.wav"
...
void PlayThread::startTimerPlay(){
    qDebug() << "startTimerPlay" << "静态方法";
    qDebug() << "p的地址" << p;
    p->startRemainPlay();
}
void PlayThread::startRemainPlay(){
    qDebug() << "startRemainPlay" << "静态方法";
    qDebug() << "this的地址" << this;
    emit startPlay();
}
//记录多少次往缓冲区填充数据播放，第一次时开始计时
int isfirst = 0;
//等待音频设备回调(会回调多次)
void pull_audio_data (void *userdata,
    ...
    isfirst += 1;
    //如果是第一次进入，开始触发定时器计时播放
    if(isfirst == 1){
        qDebug() << "第一次音频缓冲区播放" << isfirst;
        PlayThread::startTimerPlay();
    }
...
void PlayThread::run(){
   ...
   //打开设备
    ...
    double timerDuration = (double)(fileSize/BYTES_PER_SAMPLE/SAMPLE_RATE);
    qDebug() << "文件总大小" << fileSize;
    qDebug() << "音频总时长" << timerDuration;
    QString strSeconds = QString::number(timerDuration);

    int h,m,s; //获得的时间是以秒为单位的
    h = timerDuration/3600;
    m = (timerDuration-h*3600)/60;
    s = timerDuration-h*3600-m*60;
    emit dataChanged(QString("%1:%2:%3").arg(h).arg(m).arg(s));

//    double pcmTime = duration(WAVFILENAME);
//    qDebug() << "pcm时长" << pcmTime;
