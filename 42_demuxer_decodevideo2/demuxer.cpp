...
//    AVPacket pkt;
   ...
   //初始化视频信息
    ...
    av_init_packet(&pkt);
    //从文件中读取帧
    //从输入文件中读取数据-----------解码出来yuv视频文件总是少一帧或少几帧
   do{
        ret = av_read_frame(_fmtCtx,&pkt);
        qDebug() << "av_read_frame读取结果" << ret;
        if(ret == AVERROR_EOF) inEnd ++;
//        qDebug() << "读取到的文件大小和pkt大小" << pkt.size << ret;
        if(pkt.stream_index == _aStream->index){//读取到的是音频数据
//            ret = decode(_aDecodeCtx,&pkt);
//            ++audio_frame_count;
//            qDebug() << "读取到音频流的次数:audio_frame_count:" << audio_frame_count;
    ...
    qDebug() << "----------------";
//    if(inEnd == 1){//读到数据结尾处了
//        qDebug() << "读取到数据结尾处了再读取一次";
//        ret = av_read_frame(_fmtCtx,&pkt);//再读一次
//        decode(_vDecodeCtx,&pkt);
//    }
    //Flush remaining frames that are cached in the decoder
    pkt.data = nullptr;
    pkt.size = 0;
    while (decode(_vDecodeCtx, &pkt) == 0) {
...
//    ret = av_read_frame(_fmtCtx,&pkt);
//    qDebug() << "刷新缓冲区后再去读取一次:ret:" << ret;
//    decode(_vDecodeCtx,&pkt);
//    int got_frame;
//    ret = avcodec_decode_video2(_vDecodeCtx, _frame, &got_frame, &pkt);
//    qDebug() << "刷新缓冲区后再去解码一次，avcodec_decode_video2:" << ret << got_frame;
...
int Demuxer::decode(AVCodecContext *decodeCtx,AVPacket *pkt){
    ...
    while(true){
        ...
        qDebug() << "是不是有效帧" << ret << _frame->width;
...
void Demuxer::writeVideoFrame(){
    ++video_frame_count;
    qDebug() << "写入的视频帧数:video_frame_count:" << video_frame_count;
    //直接写入不正确，中间会有数据间隔
