...
#define NUMBERCHANNELS 2
  ...
  #define FILE_PATH "/Users/cloud/Documents/iOS/音视频/TestMusic/RecordToWav/"
...
void AudioThread::run(){
    ...
    //返回2，两个字节 每个样本2字节  只单独一个声道的样本占用大小，左声道或右声道
//  av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
    qDebug() << "11111111" << av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
    //返回32 表示32位 每个样本32位
    qDebug() << "22222222" <<  av_get_bits_per_sample(params->codec_id);

    //planar 左声道右声道的数据分开存储，lineSize表示左声道或右声道的总长度大小,没有乘以设备声道数
    //linesize = planar ? nb_samples * sample_size:nu_samples * sample_size * nb_channels
    ...
    qDebug() << "开始采集数据" << pkt.size;
      while(!isInterruptionRequested()){
        ...
        if( ret == 0){//读取成功
            ...
            //每当写入一点数据就更新下时间，此时知道录音录了多少时长  用定时器计算录音时长不准
            //计算录音时长  总大小/每秒钟采集多少个字节
            wavHead.dataChunkDataSize += pkt.size;
            unsigned long long ms = 1000.0 * wavHead.dataChunkDataSize/wavHead.byteRate;
            emit timeChange(ms);
...
//               wavFile.write(abyte1);
                 ...
                 wavFile.write((char *)&wavHead.dataChunkDataSize,sizeof(wavHead.dataChunkDataSize));
           }
      }
