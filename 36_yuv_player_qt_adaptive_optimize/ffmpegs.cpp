...
inStrides[0] = in.width; 在一帧yuv图片中一行的宽度是有多少个Y，即lineSize
inStrides[1] = inStrides[0] >> 1;U的数量是Y的数量的1/2
inStrides[2] = inStrides[1];V的数量和U的数量相等
*/
void FFmpegs::convertVideoRaw(RawVideoFile &in,RawVideoFile &out){
    ...
    ctx = sws_getContext(in.width,in.height,in.format,
                         out.width,out.height,out.format,
                         SWS_BILINEAR,nullptr,nullptr,nullptr);
    //每一行的lineSize，也是步长Strides
    qDebug() << "inStrides linesize" << inStrides[0] << inStrides[1] << inStrides[2] << inStrides[3];
    qDebug() << "outStrides linesize" << outStrides[0] << outStrides[1] << outStrides[2] << outStrides[3];
    if(!ctx){
        qDebug() << "sws_getContext error";
        goto end;
    }
